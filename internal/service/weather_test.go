...
func TestWeatherFetchAndCache(t *testing.T) {
	...
	ws := NewWeatherService(store, &http.Client{Timeout: 5 * time.Second})
	ws.apiBase = srv.URL
...
func TestWeatherCodeText(t *testing.T) {
	if weatherCodeText(0) != "晴" {
		t.Fatal(weatherCodeText(0))
	}
	if weatherCodeText(61) != "雨" {
		t.Fatal(weatherCodeText(61))
	}
}
