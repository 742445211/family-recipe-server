...
| tips | string | 否 | 小贴士 |
| is_public | bool | 否 | 不传时默认 `false` |

创建/更新时会调用微信 **msgSecCheck** 内容安全 API 检测文本字段（菜名、分类、食材、步骤、小贴士等）。违规时返回：
...
更新菜谱（需登录）。仅更新非零值字段；须属于当前家庭。同样经过 msgSecCheck 内容安全检测。
...
变更记录：
- 2026-06-24 接入微信 msgSecCheck；is_public 默认 false
- 2026-06-12 初版（含 seasonings 字段说明）
