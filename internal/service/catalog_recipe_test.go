...
func TestNormalizeNameKey(t *testing.T) {
	if got := NormalizeNameKey("  番茄炒蛋  "); got != "番茄炒蛋" {
		t.Fatalf("got %q", got)
	}
	if got := NormalizeNameKey("Tomato Egg"); got != "tomatoegg" {
		t.Fatalf("got %q", got)
	}
}

func TestCatalogContentHashDedup(t *testing.T) {
	h1 := CatalogContentHash(`[{"name":"a"}]`, "[]", `["step"]`)
	h2 := CatalogContentHash(`[{"name":"a"}]`, "[]", `["step"]`)
	if h1 != h2 {
...
func TestCatalogSaveFromAIAndLookup(t *testing.T) {
	...
	svc := NewCatalogRecipeService(db, nil, nil)
	in := AIRecommendItemInput{
		Name: "番茄炒蛋", Category: "家常菜", Difficulty: "easy", CookTime: 15,
		Ingredients: `[{"name":"番茄","amount":"2"}]`, Seasonings: "[]", Steps: `["炒"]`, Tips: "快",
	}
	rec, err := svc.SaveFromAI(in, CatalogSourceAISearch, "经典做法")
	if err != nil || rec.ID == 0 {
	...
	list, err := svc.LookupByNameKey(NormalizeNameKey("番茄炒蛋"))
	if err != nil || len(list) != 1 {
		t.Fatalf("list=%+v err=%v", list, err)
	}

	dup, err := svc.SaveFromAI(in, CatalogSourceAISearch, "重复")
...
func TestCatalogLookupOrGenerateHitNoAI(t *testing.T) {
	...
	rl := NewAIRateLimitService(store)
	svc := NewCatalogRecipeService(db, nil, rl)

	in := AIRecommendItemInput{Name: "红烧肉", Category: "荤菜", Ingredients: "[]", Seasonings: "[]", Steps: `["煮"]`}
	if _, err := svc.SaveFromAI(in, CatalogSourceAISearch, "经典做法"); err != nil {
		t.Fatal(err)
	}

	result, err := svc.LookupOrGenerate(context.Background(), 1, "红烧肉", CatalogLookupOpts{})
	if err != nil {
		t.Fatal(err)
	}
	if result.Generated {
...
func TestCatalogLookupOrGenerateCallsAI(t *testing.T) {
	...
	store := cache.NewRedisCache(mr.Addr(), "", 0)
	ai := NewAIServiceWithClient(srv.Client())
	ai.baseURL = srv.URL
	svc := NewCatalogRecipeService(db, ai, NewAIRateLimitService(store))

	result, err := svc.LookupOrGenerate(context.Background(), 9, "新菜A", CatalogLookupOpts{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCatalogNewVariantAddsSecondRow(t *testing.T) {
	...
	if _, err := svc.LookupOrGenerate(context.Background(), 1, "鱼香肉丝", CatalogLookupOpts{}); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.LookupOrGenerate(context.Background(), 1, "鱼香肉丝", CatalogLookupOpts{NewVariant: true}); err != nil {
		t.Fatal(err)
	}
	list, _ := svc.LookupByNameKey(NormalizeNameKey("鱼香肉丝"))
	if len(list) != 2 {
