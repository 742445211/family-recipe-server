...
func TestCreateWecomChannelResolvesMobile(t *testing.T) {
	...
	ch, err := svc.Create(user.ID, ChannelInput{Channel: model.ChannelWecomWorkbench, Secret: "13800138000"})
	if err != nil {
...
func TestCreateWecomChannelKeepsUserid(t *testing.T) {
	...
	ch, err := svc.Create(user.ID, ChannelInput{Channel: model.ChannelWecomWorkbench, Secret: "lisi"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if resolver.called {
...
func TestUpdateWecomChannelResolvesMobile(t *testing.T) {
	...
	if err := svc.Update(user.ID, ch.ID, ChannelInput{Secret: "13912345678"}); err != nil {
