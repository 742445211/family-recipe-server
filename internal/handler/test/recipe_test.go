...
func setupRecipeRouter(t *testing.T) (*gin.Engine, *handler.RecipeHandler, uint64, uint64, *gorm.DB) {
	t.Helper()
...
func TestRecipeListPagination(t *testing.T) {
	r, _, userID, familyID, _ := setupRecipeRouter(t)
...
func TestRecipeGetRejectsOtherFamilyPrivate(t *testing.T) {
	r, _, userID, familyID, db := setupRecipeRouter(t)
	var privateRecipe model.Recipe
	db.Where("name = ?", "外来私有菜谱").First(&privateRecipe)
...
func TestRecipeGetAllowsPublicRecipe(t *testing.T) {
	...
	db.Where("name = ?", "外来公开菜谱").First(&publicRecipe)
