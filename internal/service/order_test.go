...
func TestOrderAddAndList(t *testing.T) {
	...
	// Add
	order, err := svc.Add(familyID, recipe.ID, userID, "2026-05-01", "少放花椒", 2)
	if err != nil {
		t.Fatalf("点菜失败: %v", err)
	}
	if order.Quantity != 2 {
	...
	// Add another
	svc.Add(familyID, recipe.ID, userID, "2026-05-01", "", 1)

	// Get by date
	orders, err := svc.GetByDate(familyID, "2026-05-01")
	if err != nil {
		t.Fatalf("查询失败: %v", err)
	}
	if len(orders) != 2 {
	...
	orders, _ = svc.GetByDate(familyID, "2026-05-02")
	if len(orders) != 0 {
	...
	orders, _ = svc.GetByDate(familyID, "2026-05-01")
	if len(orders) != 1 {
