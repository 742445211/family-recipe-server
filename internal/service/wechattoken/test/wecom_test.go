...
func TestWecomGetUseridByMobile(t *testing.T) {
	...
	userid, err := wechattoken.NewWecomToken().GetUseridByMobile("13800138000")
	if err != nil {
...
func TestWecomGetUseridByMobileNotFound(t *testing.T) {
	...
	if _, err := wechattoken.NewWecomToken().GetUseridByMobile("13800138000"); err == nil {
		t.Fatal("查询失败应返回错误")
	}
}
...
func TestWecomTokenTrimsAPIBaseSlash(t *testing.T) {
	...
	tok, err := wechattoken.NewWecomToken().GetAccessToken()
	if err != nil || tok != "ok" {
