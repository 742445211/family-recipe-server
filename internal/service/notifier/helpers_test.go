...
func TestFormatIngredientsInvalidJSON(t *testing.T) {
	if FormatIngredients("{bad") != "" {
		t.Fatal("非法 JSON 应返回空")
	}
}

func TestTruncateRunes(t *testing.T) {
	if truncateRunes("你好世界", 10) != "你好世界" {
		t.Fatal("未超长不应截断")
	}
	if truncateRunes("你好世界abcdef", 4) != "你好世界..." {
		t.Fatalf("got %q", truncateRunes("你好世界abcdef", 4))
	}
}
...
func TestMaskSecret(t *testing.T) {
	...
	if mask("abcdefghij") != "abc***hij" {
		t.Fatalf("got %q", mask("abcdefghij"))
	}
}

func TestTrimSlash(t *testing.T) {
	if trimSlash("https://api.test/") != "https://api.test" {
		t.Fatal(trimSlash("https://api.test/"))
	}
}
