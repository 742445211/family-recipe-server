...
func TestWeChatSubscribeMissingOpenID(t *testing.T) {
	n := NewWeChatSubscribeNotifier(true, stubTokenProvider{token: "tok"})
	_, err := n.Send(context.Background(), NotificationMessage{}, NotificationTarget{})
	if err == nil {
...
func TestWeChatSubscribeUsesMessageOpenID(t *testing.T) {
	...
	n := NewWeChatSubscribeNotifier(false, stubTokenProvider{token: "tok"})
	if n.Enabled() {
...
func TestWeChatSubscribeChannelName(t *testing.T) {
	n := NewWeChatSubscribeNotifier(true, stubTokenProvider{})
