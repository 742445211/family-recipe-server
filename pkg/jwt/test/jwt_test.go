...
func TestGenerateAndParse(t *testing.T) {
	secret := "test-secret"
	token, err := jwt.Generate(secret, 1, 1, "test-openid", 1)
	if err != nil {
...
func TestParseInvalidToken(t *testing.T) {
	_, err := jwt.Parse("secret", "invalid-token")
	if err == nil {
...
func TestParseExpiredToken(t *testing.T) {
	...
	_, err := jwt.Parse(secret, tokenStr)
	if err == nil {
...
func TestParseWrongSecret(t *testing.T) {
	token, _ := jwt.Generate("secret-a", 1, 1, "test", 0)
	_, err := jwt.Parse("secret-b", token)
	if err == nil {
