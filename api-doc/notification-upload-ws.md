...
上传图片（需登录）。支持 jpg/jpeg/png/webp/gif；单张最大 **10MB**。启用内容安全时，超过 1MB 的图片由服务端压缩后再调用微信 **img_sec_check**，通过检测后仍上传**原图**至 OSS；关联 `recipe_id` 时树莓派异步压缩封面流程不变。
...
**错误：** 400 格式/大小不合法 / 内容违规 / 图片压缩失败；菜谱不存在或非本家庭
...
变更记录：
- 2026-06-24 大图由服务端压缩后送 img_sec_check，原图仍上传 OSS
