...
func TestParseAIRecommendJSON(t *testing.T) {
	...
	items, err := service.ParseAIRecommendJSONForTest(raw)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAIRecommendServiceRecommend(t *testing.T) {
	...
	weather := service.NewWeatherService(store, nil)
	ctxSvc := service.NewAIContextService(db, weather)
	ai := service.NewAIServiceWithClient(&http.Client{})
	service.SetAIServiceBaseURLForTest(ai, srv.URL)
	svc := service.NewAIRecommendService(db, store, ai, ctxSvc, service.NewAIRateLimitService(store))
	...
	var draft service.AIRecipeDraft
	if err := store.GetJSON(context.Background(), service.AIItemKeyForTest(result.Items[0].ItemID), &draft); err != nil {
		t.Fatal(err)
	}
...
func TestAIRecommendUsesMealOverride(t *testing.T) {
	...
	ai := service.NewAIServiceWithClient(&http.Client{})
	service.SetAIServiceBaseURLForTest(ai, srv.URL)
	svc := service.NewAIRecommendService(db, store, ai, service.NewAIContextService(db, service.NewWeatherService(store, nil)), service.NewAIRateLimitService(store))
...
func TestFilterNewDishesOnly(t *testing.T) {
	...
	inputs := []service.AIRecommendItemInput{
		{Name: "红烧肉"},
		{Name: "鱼香肉丝"},
		{Name: "番茄炒蛋"},
		{Name: "蒜蓉西兰花"},
	}
	got := service.FilterNewDishesOnlyForTest(inputs, existing)
...
func TestAIRecommendEnsuresNewCategory(t *testing.T) {
	...
	var draft service.AIRecipeDraft
	_ = store.GetJSON(context.Background(), service.AIItemKeyForTest(result.Items[0].ItemID), &draft)
...
func TestImportAIRecipeFromCache(t *testing.T) {
	...
	_ = store.SetJSON(context.Background(), service.AIItemKeyForTest(itemID), draft, 0)

	svc := service.NewAIRecommendService(db, store, nil, nil, nil)
...
func TestAddOrderFromAIItem(t *testing.T) {
	...
	svc := service.NewAIRecommendService(db, store, nil, nil, nil)
	order, err := svc.AddOrderFromItem(context.Background(), itemID, fid, uid, service.AddOrderFromAIRequest{
...
func TestGetAIItemWrongFamily(t *testing.T) {
	...
	t.Cleanup(mr.Close)
	store := cache.NewRedisCache(mr.Addr(), "", 0)
	_ = store.SetJSON(context.Background(), service.AIItemKeyForTest("x"), service.AIRecipeDraft{FamilyID: 1, Name: "A"}, 0)
	svc := service.NewAIRecommendService(nil, store, nil, nil, nil)
